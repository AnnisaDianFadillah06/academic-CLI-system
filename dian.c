...
// Fungsi untuk menampilkan data Mahasiswa dengan garis tiap baris dan kolom
void displayMahasiswaData(const char *className) {
    ...
    // Tambahkan garis horizontal di antara header dan data
    printf("-------------------|--------------------\n");
    ...
    for (i = 0; i < numMahasiswa; i++) {
        ...
        if (strcmp(trimmedKelas, className) == 0) {
            ...
            // Tambahkan garis horizontal di antara setiap baris data
            printf("-------------------|--------------------\n");
        }
    }
    ...
    sprintf(nilaiMahasiswas[*numNilai].kodeMatkul, "%s", loggedInKodeMatkul);
     ...
     for (i = 0; i < numMahasiswa; i++) {
        if (strcmp(mahasiswas[i].nim, nim) == 0) {
            ...
            printf("Input Nilai: ");
            scanf("%d", &nilai);
            simpanNilai(nim, nilai, loggedInKodeMatkul, numNilai, mahasiswas[i]);
...
//    printf("Trying to open file: %s\n", namaFile);
    ...
    file = fopen(namaFile, "r+");  // Open in read/write mode
    if (file == NULL) {
        // Handle errors opening/creating file
        perror("Error opening/creating file");
        exit(EXIT_FAILURE);
    }

    // Mencari NIM dalam file
    int found = 0;
    long currentPos = 0;
    while (fscanf(file, "%s %s %s %s %d", nilai.nim, nilai.username, nilai.kelas, nilai.kodeMatkul, &nilai.nilai) == 5) {
        if (strcmp(nim, nilai.nim) == 0) {
            found = 1;
            fseek(file, currentPos, SEEK_SET); // Move to the beginning of the line

            int newNilaiLength = snprintf(NULL, 0, "%d", newNilai);
            fprintf(file, "%s %s %s %s %-*d\n", nilai.nim, nilai.username, nilai.kelas, nilai.kodeMatkul, newNilaiLength, newNilai);
			printf("Nilai berhasil diupdate.\n");
            break;
        }
        currentPos = ftell(file);
    }
    // Jika NIM tidak ditemukan, tulis nilai baru ke file
    if (!found) {
        fseek(file, 0, SEEK_END);
        sanitizeString(mahasiswa.kelas);
        fprintf(file, "%s %s %s %s %-*d\n", nim, mahasiswa.username, mahasiswa.kelas, kodeMatkul, snprintf(NULL, 0, "%d", newNilai), newNilai);
...
//    }
    ...
    printf("==================================================================\n");
   	printf("                         HALAMAN DOSEN                            \n");
   	printf("==================================================================\n");
   	printf("Daftar Menu Dosen\n");
    ...
    printf("Input pilihan menu (1/2/3): ");
    scanf("%d", &choice);
...
//	            printf("Selected class: %s\n", classChoice); // For debugging
	    ...
	    case 2 : {
			...
			generateRandomSchedule(dosens, numDosen, namaFile);
